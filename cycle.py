...
sql = """
(select fiat.id, amount, target, user_id, cur_id, pt_id, parent_id, amount - target as need, (amount - target) / rate as need_usd
 from fiat
...
async def tick(pairs: [Pair]):
    ...
    fd = {}
    for fiat in fiats:
        fd[fiat['cur_id']] = fd.get(fiat['cur_id'], []) + [(fiat['pt_id'], fiat['need'], fiat['parent_id'], fiat['user_id'])]
    for pair in pairs:
